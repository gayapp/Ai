...
| 时钟校准记录 | 本会话发现：系统 reminder 给的"今天日期"与 D1 wall clock 偶尔会差几天到 12 天。**以 D1 `datetime('now')` 为真**做容量/时间换算，避免基于错误日期得出"紧急 vs 不紧急"的错误结论。06-15 那次 M28 实施时误判涨速失控（实际是 06-27，IRC 峰值已过 16 天，温和稳态 35MB/天）| 行为约束记录，不需 prod 改动 |
