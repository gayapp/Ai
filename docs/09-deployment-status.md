...
- **健康检查**：`GET /health` → 200
- **当前 Worker 版本**：`ea437110-4f3c-41aa-9668-1f58cbfdc56e`（最近一次生产 deploy）
- **D1 migrations**：`0001` 到 `0012` 已应用
...
| avatar | Gemini Vision | `gemini-2.5-flash-lite` |
...
Gemini 模型可在 Admin UI `/providers` 或 Admin API `PATCH /admin/providers/models` 热切换；默认使用 `gemini-2.5-flash-lite`。
...
- Admin 审计日志：`admin_audit_logs` 表、`/admin/audit` API、Admin UI `#/audit`
