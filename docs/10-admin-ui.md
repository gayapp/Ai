...
列表默认展示当前时间窗内的所有状态，时间窗支持 `1h` / `24h` / `7d` /
`all`；如果从运营页跳入时 URL 带有
...
- `provider_strategy=grok` 会让 analyze 线只走 xAI / Grok，不 fallback 到 Gemini；xAI 临时故障时请求保持 `pending`，等待队列重试和 pending sweep 追赶。`auto` 下 `media_analysis` 默认 Gemini 优先、xAI fallback，`media_intro` 默认 xAI 优先、Gemini fallback。
