-- 0017 · post 审核 prompt v3：二维码无条件禁止（独立成条，不适用白名单例外）
-- 只追加：停用 post/grok v2，插入 v3 为 active。

UPDATE prompts SET is_active = 0 WHERE biz_type = 'post' AND provider = 'grok' AND version = 2;
...
- 命中本细则时，evidence 必须写明在第几张图见到的具体证据，如「第2张图见到二维码」或「第2张图见到 @xxx 账号水印」（账号文本照抄）。
